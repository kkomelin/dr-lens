#!/usr/bin/env bash
# Prepares a Chrome Web Store upload: validates the extension, runs the
# tests, and packs only the runtime files into dist/dr-lens-v<version>.zip.
set -euo pipefail
...
OUT="dist/dr-lens-v${VERSION}.zip"
mkdir -p dist
rm -f "$OUT"
...
Pre-publish checklist:
  ...
  2. Privacy practices tab: disclose hostname-only requests to
